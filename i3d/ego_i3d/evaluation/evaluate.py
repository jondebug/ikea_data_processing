...
import utils
import eval_utils
from DataLoader import HololensStreamRecClipDataset as Dataset
...
def trim_annotations_to_round_clip_length(gt_labels, clip_length=32):
    trimmed_labels = []
    for annotation_vec in gt_labels:
        num_frames = len(annotation_vec)
        target_len = num_frames - num_frames%clip_length
        trimmed_labels.append(annotation_vec[:target_len])
...
gt_json_path = os.path.join(args.dataset_path, "indexing_files", 'db_gt_annotations.json')
# dataset = Dataset(args.dataset_path, action_segments_filename=gt_json_path)
dataset = Dataset(args.dataset_path, dataset="test")

gt_labels = dataset.action_labels
gt_labels = trim_annotations_to_round_clip_length(gt_labels=gt_labels, clip_length=32)
...
print(classification_score_str)
# Compute accuracy
acc1_per_vid = []
acc3_per_vid = []
...
fig, ax = eval_utils.plot_confusion_matrix(cm=c_matrix,
